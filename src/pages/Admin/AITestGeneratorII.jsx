...
function AITestGeneratorII() {
  ...
  const [apiKey, setApiKey] = useState('AIzaSyAA_0U_t7k61MkTMRvZPnmc7KNRQHLITnY');
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      console.log('Extracted text:', extractedText.length, 'chars. Preview:', extractedText.substring(0, 300));
            ...
            // STEP 2: Local deterministic question structuring (no API needed)
      setProgress('Structuring questions from extracted text (local parser)...');
      const allLines = extractedText.split('\n').map(l => l.trim()).filter(Boolean);
      const sectionOrder = [
        { name: 'Physics Section 1', start: 1, end: 20, type: 'single_correct' },
        { name: 'Physics Section 2', start: 21, end: 25, type: 'numerical' },
        { name: 'Chemistry Section 1', start: 26, end: 45, type: 'single_correct' },
        { name: 'Chemistry Section 2', start: 46, end: 50, type: 'numerical' },
        { name: 'Mathematics Section 1', start: 51, end: 70, type: 'single_correct' },
        { name: 'Mathematics Section 2', start: 71, end: 75, type: 'numerical' },
      ];
      const questions = [];
      let cQ = null, cO = null;
      const qRx = /^(?:(?:Q\.?|Question)\s*)?(\d{1,2})[.)\s]/i;
      const oRx = /^\(?([1-4A-Da-d])[.)\s]/;
      for (const line of allLines) {
        const qm = line.match(qRx);
        if (qm && parseInt(qm[1]) >= 1 && parseInt(qm[1]) <= 75) {
          if (cQ) questions.push(cQ);
          const num = parseInt(qm[1]);
          const sec = sectionOrder.find(s => num >= s.start && num <= s.end) || sectionOrder[0];
          cQ = { questionNumber: num, sectionName: sec.name, type: sec.type, text: line.replace(qm[0], '').trim(), options: [], correctOption: '', needsScreenshot: false };
          cO = null; continue;
        }
        const om = line.match(oRx);
        if (om && cQ && cQ.type === 'single_correct' && cQ.options.length < 4) {
          const rawId = (om[1] || '').toUpperCase();
          const id = rawId === '1' ? 'A' : rawId === '2' ? 'B' : rawId === '3' ? 'C' : rawId === '4' ? 'D' : rawId;
          cO = { id, text: line.replace(om[0], '').trim(), needsScreenshot: false };
          cQ.options.push(cO); continue;
        }
        if (cO && cQ) cO.text += ' ' + line;
        else if (cQ) cQ.text += ' ' + line;
      }
      if (cQ) questions.push(cQ);
      ...
      for (const sec of sectionOrder) {
        for (let n = sec.start; n <= sec.end; n++) {
          if (!seen.has(n)) {
            const stub = { questionNumber: n, sectionName: sec.name, type: sec.type, text: '[Q' + n + ' - edit manually]', options: sec.type === 'single_correct' ? [{id:'A',text:'Option A',needsScreenshot:false},{id:'B',text:'Option B',needsScreenshot:false},{id:'C',text:'Option C',needsScreenshot:false},{id:'D',text:'Option D',needsScreenshot:false}] : [], correctOption: '', needsScreenshot: false };
            questions.push(stub);
          }
        }
      }
      ...
      setProgress('Complete! ' + questions.length + ' questions structured.');
    } catch (err) {
      console.error(err);
    ...
    <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
      <div style={{ marginBottom: '30px' }}>
        ...
        <p style={{ color: 'var(--dash-text-muted)' }}>
          Upload a PDF. The local parser will structure 75 questions. Use the API only for percentile extraction. No API key needed for test generation.
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: generatedTest ? '350px 1fr' : '1fr 1fr', gap: '30px', transition: 'all 0.3s' }}>
        <div style={{ backgroundColor: 'var(--dash-surface)', padding: '30px', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', height: '800px', overflowY: 'auto' }}>
          ...
          >
            ...
            {loading ? 'Parsing PDF locally...' : 'Generate Test'}
          </button>
